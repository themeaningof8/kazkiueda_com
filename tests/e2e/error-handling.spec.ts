...
test.describe('エラーハンドリング', () => {
  test('存在しないページで404が表示される', async ({ page }) => {
    ...
    const has404 = pageContent.includes('404') || pageContent.includes('Not Found') || pageContent.includes('記事が見つかりません');
  ...
  test('不正なプレビューシークレットでアクセスすると403', async ({ page }) => {
    // 不正なシークレットでアクセス
    const response = await page.goto('/preview?collection=posts&slug=test-slug&previewSecret=invalid');
