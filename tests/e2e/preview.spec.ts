...
test.describe('プレビュー機能', () => {
  ...
  test('認証なしアクセスの拒否 - 不正なシークレット', async ({ page }) => {
    // 不正なプレビューシークレットでアクセス
    const response = await page.goto('/preview?collection=posts&slug=any-slug&previewSecret=invalid-secret');
  ...
  test('Open Redirect対策 - 外部URL拒否', async ({ page }) => {
    // 外部URLをpathパラメータに指定
    const response = await page.goto(`/preview?collection=posts&slug=test-post&path=http://evil.com&previewSecret=${previewSecret}`);
  ...
  test('Open Redirect対策 - スキームなし外部URL拒否', async ({ page }) => {
    // //evil.com 形式のURLを拒否
    const response = await page.goto(`/preview?collection=posts&slug=test-post&path=//evil.com&previewSecret=${previewSecret}`);
  ...
  test('Open Redirect対策 - 許可されないパス拒否', async ({ page }) => {
    // /posts/ 配下以外のパスを拒否
    const response = await page.goto(`/preview?collection=posts&slug=test-post&path=/admin/dashboard&previewSecret=${previewSecret}`);
  ...
  test('slug形式検証 - 不正なslug拒否', async ({ page }) => {
    // スペースを含む不正なslug
    const response = await page.goto(`/preview?collection=posts&slug=invalid slug&previewSecret=${previewSecret}`);
  ...
  test('コレクション検証 - サポートされないコレクション拒否', async ({ page }) => {
    // posts以外はサポートされない
    const response = await page.goto(`/preview?collection=users&slug=test-user&previewSecret=${previewSecret}`);
