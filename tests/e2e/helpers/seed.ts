...
export async function cleanDatabase(payload: Payload, options?: {
  keepUsers?: boolean;
}) {
  // 投稿を順次削除（デッドロック回避のため）
  try {
  ...
  if (!options?.keepUsers) {
    try {
      const users = await payload.find({ collection: 'users', where: { email: { contains: 'e2e-' } }, limit: 10 });
...
export async function seedDatabase(payload: Payload) {
  ...
  return {
    ...
    posts,
    // E2ETestData形式で返す
    publishedPosts: posts.filter(p => p._status === 'published').map(p => ({
      id: p.id,
      slug: p.slug || '',
      title: p.title,
    })),
