...
async function globalSetup() {
  ...
  const { createTestUser, createBulkTestPosts, createTestPost } = await import(
    "../../../src/test/helpers/factories"
  );
  ...
  try {
    ...
    try {
      const existingUsers = await payload.find({
        collection: "users",
        where: {
          or: [{ email: { equals: "e2e-admin@test.com" } }, { email: { contains: "e2e-admin-" } }],
        },
      });
