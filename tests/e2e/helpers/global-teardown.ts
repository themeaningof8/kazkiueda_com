...
async function globalTeardown() {
  ...
  try {
    ...
    if (testData.version !== "1.0.0") {
      console.warn(
        `⚠️ テストデータのバージョンが一致しません。期待: 1.0.0, 実際: ${testData.version}`,
      );
    }
    ...
    try {
      ...
      for (const post of testData.publishedPosts) {
        await payload
          .delete({ collection: "posts", id: post.id })
          .catch((err: unknown) => console.warn(`記事削除失敗 (ID: ${post.id}):`, err));
      }

      // 下書き記事を削除
      await payload
        .delete({ collection: "posts", id: testData.draftPost.id })
        .catch((err: unknown) => console.warn(`下書き削除失敗:`, err));

      // 管理者ユーザーを削除
      await payload
        .delete({ collection: "users", id: testData.adminUser.id })
