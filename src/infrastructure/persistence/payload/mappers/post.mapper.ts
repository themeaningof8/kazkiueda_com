...
import { Post, type PostContent, type PostTag } from "@/domain/entities/post.entity";
import { Slug } from "@/domain/value-objects/slug.vo";
import type { Post as PayloadPost } from "@/payload-types";

/**
 * Payload Post型からドメインPostエンティティに変換
 */
export class PostMapper {
  ...
  static toDomain(payloadPost: PayloadPost): Post {
    // スラグの処理
    const slug = payloadPost.slug
      ? Slug.create(payloadPost.slug)
      : Slug.fromTitle(payloadPost.title);
    ...
    // タグの変換
    const tags: PostTag[] = payloadPost.tags
   ...
   * (作成・更新時に使用)
   */
  static toPayload(post: Post): Partial<PayloadPost> {
    return {
      title: post.title,
      slug: post.slug,
   ...
   */
  static toDomainList(payloadPosts: PayloadPost[]): Post[] {
    return payloadPosts.map((p) => PostMapper.toDomain(p));
  }

  /**
   * Slugの配列を文字列配列に変換
   */
  static slugsToStrings(posts: Post[]): string[] {
