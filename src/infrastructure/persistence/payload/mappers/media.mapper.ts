 ...
 */
export function toMediaDomain(payloadMedia: PayloadMedia): Media {
  ...
  // サイズ情報の変換
  const sizes = payloadMedia.sizes
    ? {
 ...
 * (更新時に使用)
 */
export function toMediaPayload(media: Media): Partial<PayloadMedia> {
  return {
    alt: media.alt || null,
 ...
 */
export function toMediaDomainList(payloadMediaList: PayloadMedia[]): Media[] {
  return payloadMediaList.map(toMediaDomain);
}
