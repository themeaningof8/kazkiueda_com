...
describe("Performance Tests", () => {
  ...
  describe("Lightweight Performance Tests (CI Compatible)", () => {
    test("should complete findPublishedPostSlugs within 500ms for 100 posts", async () => {
    ...
    test("should complete getPosts pagination within 300ms for 100 posts", async () => {
      const payload = await getTestPayload(payloadKey);
      const user = await createTestUser(payload);
  ...
  // CI環境ではスキップする重いテスト
  describe.skipIf(process.env.CI)("Heavy Performance Tests (Local Only)", () => {
