...
describe("env", () => {
  ...
  describe("validateEnv", () => {
    test("有効な環境変数で正常に検証される", async () => {
      ...
      (process.env as any).PAYLOAD_SECRET =
        "test-secret-key-that-is-long-enough-for-validation-32-chars";
      (process.env as any).DATABASE_URL = "postgresql://user:password@localhost:5432/testdb";
    ...
    test("PAYLOAD_SECRETがデフォルト値の場合エラー", async () => {
      (process.env as any).NODE_ENV = "test";
      (process.env as any).PAYLOAD_SECRET = "your-secret-key";
      (process.env as any).DATABASE_URL = "postgresql://user:password@localhost:5432/testdb";
    ...
    test("R2設定が部分的に設定されている場合エラー", async () => {
      ...
      (process.env as any).PAYLOAD_SECRET =
        "test-secret-key-that-is-long-enough-for-validation-32-chars";
      (process.env as any).DATABASE_URL = "postgresql://user:password@localhost:5432/testdb";
      (process.env as any).S3_BUCKET = "test-bucket";
      // 他のR2設定を未設定
    ...
    test("R2設定が全て設定されている場合正常", async () => {
      ...
      (process.env as any).DATABASE_URL = "postgresql://user:password@localhost:5432/testdb";
      (process.env as any).S3_BUCKET = "test-bucket";
      (process.env as any).S3_ENDPOINT = "https://test.r2.cloudflarestorage.com";
      (process.env as any).S3_ACCESS_KEY_ID = "test-key";
      (process.env as any).S3_SECRET_ACCESS_KEY = "test-secret";
      (process.env as any).S3_REGION = "auto";
    ...
    test("本番環境でResend設定が全て設定されている場合正常", async () => {
      ...
      (process.env as any).PAYLOAD_SECRET =
        "test-secret-key-that-is-long-enough-for-validation-32-chars";
      (process.env as any).DATABASE_URL = "postgresql://user:password@localhost:5432/testdb";
      (process.env as any).RESEND_API_KEY = "test-api-key";
      (process.env as any).RESEND_FROM_EMAIL = "test@example.com";
      (process.env as any).RESEND_FROM_NAME = "Test App";
    ...
    test("NEXT_PUBLIC_SITE_URLが不正なURLの場合エラー", async () => {
      ...
      (process.env as any).PAYLOAD_SECRET =
        "test-secret-key-that-is-long-enough-for-validation-32-chars";
      (process.env as any).DATABASE_URL = "postgresql://user:password@localhost:5432/testdb";
      (process.env as any).NEXT_PUBLIC_SITE_URL = "not-a-url";
