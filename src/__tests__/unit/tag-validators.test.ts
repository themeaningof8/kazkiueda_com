...
describe("validateTag", () => {
  ...
  test("長すぎるタグがエラー", () => {
    ...
    expect(validateTag(longTag)).toBe(`タグは${VALIDATION_LIMITS.TAG_MAX_LENGTH}文字以内で入力してください`);
  });
  ...
  test("境界値テスト: TAG_MAX_LENGTH + 1文字はNG", () => {
    ...
    expect(validateTag(tooLongTag)).toBe(`タグは${VALIDATION_LIMITS.TAG_MAX_LENGTH}文字以内で入力してください`);
  });
});

describe("validateTagsArray", () => {
  test("正常な配列が通る（重複なし）", () => {
    expect(validateTagsArray([
      { tag: "react" },
      { tag: "typescript" },
    ])).toBe(true);
  });
  ...
  test("tagプロパティが非文字列のみの場合、重複チェックされない", () => {
    expect(validateTagsArray([
      { tag: 123 },
      { tag: 123 },
    ])).toBe(true); // 文字列として抽出されないため、重複チェック対象外
  });
