...
export function ErrorPage({ error, onRetry }: ErrorPageProps) {
  ...
  return (
    <div className="flex min-h-[400px] flex-col items-center justify-center px-4 py-16 text-center">
      ...
      <h1 className="mb-4 text-2xl font-bold text-gray-900 dark:text-gray-100">{content.title}</h1>

      <p className="mb-8 max-w-md text-gray-600 dark:text-gray-400">{content.message}</p>

      {error?.message && (
        <p className="mb-8 max-w-md text-sm text-gray-500 dark:text-gray-500">{error.message}</p>
      )}
